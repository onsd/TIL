...
image = cv2.imread("./test/131954.png")
...
for r in boundingRect:
    (x, y, w, h) = r
    d = imutils.resize(threshold[y:y + h, x:x + w],
                       height=standard, width=standard)
    print(d.shape)
    tmp_h, tmp_w = d.shape
    ...
    print(str(ptop) + '-' + str(pbuttom) +
          '-' + str(pleft) + '-' + str(pright))
    if (ptop > 0 and pbuttom > 0 and pleft > 0 and pright > 0):
        constant = cv2.copyMakeBorder(
            d, ptop, pbuttom, pleft, pright, cv2.BORDER_CONSTANT, value=[255, 255, 255])
